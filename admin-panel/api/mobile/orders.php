...
function oneSignalExternalIdAllowedForOrders($value)
{
    $value = trim((string) $value);
    if ($value === '') {
        return false;
...
function oneSignalAliasMapForOrderTarget($externalUserId)
{
    ...
    $aliases = [];
    $externalIds = [];
    if (oneSignalExternalIdAllowedForOrders($external)) {
    ...
    if (preg_match('/^provider_(\d+)$/', $external, $providerMatch)) {
        ...
        if (oneSignalExternalIdAllowedForOrders($userMatch[1])) {
...
function oneSignalTagFiltersForOrderTarget($externalUserId)
{
    $external = trim((string) $externalUserId);
    if ($external === '') {
        return [];
...
function sendOneSignalToExternalUser($externalUserId, $title, $body, $data = [], array $subscriptionIds = [])
{
    ...
    $aliasPayload['include_aliases'] = oneSignalAliasMapForOrderTarget($external);
    ...
    $tagFilters = oneSignalTagFiltersForOrderTarget($external);
    $tagResult = ['error' => ''];
    if (!empty($tagFilters)) {
        $tagPayload = $basePayload;
        $tagPayload['filters'] = $tagFilters;

        $tagResult = postOneSignalPayloadForOrders($tagPayload);
        if (oneSignalTargetDeliveredForOrders($tagResult)) {
            return true;
        }
    }
